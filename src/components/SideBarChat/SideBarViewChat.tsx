...
interface Props {
    ...
    onEdit?: (model?: ChatSession) => void
}

export function SideBarViewChat(props: Props) {
    ...
    return (
        <>
            <a className="cursor-pointer flex flex-1 p-1 items-center"
               title={props.chat?.name}>{_.truncate(props.chat?.name, {length: 16})}</a>

            <a className="cursor-pointer hover:bg-gray-200/10" onClick={() => props.onEdit?.(props.chat)}>
                <svg
                    stroke="currentColor"
                    fill="none"
                    strokeWidth={2}
