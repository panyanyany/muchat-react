...
import {delChat, setAppPresetPrompt, setPresetPrompt} from "../../store/appSlice";
import {useDispatch, useSelector} from "react-redux";

export interface IChatFnMenuProps {
    show: boolean,
    onClose?: () => void
    chat?: ChatSession
}

export function ChatFnMenu(props: IChatFnMenuProps) {
    ...
    return <>
                                    ...
                                    <div className="sm:flex sm:items-start">
                                        ...
                                        <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            <Dialog.Title as="h3"
                                                ...
                                                {i18nUtil.t('菜单')}<br className="block md:hidden"/>
                                            </Dialog.Title>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <a className="p-2 hover:bg-gray-200 cursor-pointer">菜单1</a>
                                    {
                                        ...
                                        <a className="p-2 hover:bg-gray-200 cursor-pointer" onClick={() => {
                                            const res = confirm('确定删除会话吗？')
                                            if (res) {
                                                dispatch(delChat({chatId: props.chat?.id}))
                                                props.onClose?.()
                                            }
                                        }}>删除会话</a>
                                    }
        ...
        <PromptPanel show={prompt} onClose={() => setPrompt(false)}
                     pp={props.chat?.presetPrompt || app.presetPrompt}
                     onChange={(pp) => {
                         if (props.chat?.id) {
